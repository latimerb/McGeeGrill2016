...
def write_file(spikes_path, stims):
	...
	with open(spikes_path, 'w') as csvfile:
		...
		csvwriter.writerow(['node_ids', 'timestamps'])  # Create csv header
		for node_id, timestamps in stims.items():
			...
			for t in timestamps:
				csvwriter.writerow([node_id, t])  # write head node/time pair to a row.
